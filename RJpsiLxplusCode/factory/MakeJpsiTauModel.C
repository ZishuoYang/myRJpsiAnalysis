...
const double DataSize = 0.1034; // Relative luminosity of fit sample to control sample (which is about 1/fb)
const double MisIDSize = 1.0277*(213.0-7.0)/213.0;
const double RelLumi = DataSize/MisIDSize;
const double expMu = 5.5e4*DataSize/3.0; // in total sample; multiply by rellumi
const double expRawR = 0.15;
const double expComb = 2.7e4*DataSize/3.0;
...
const int numToys = 100;
const bool BeestonBarlow = false;
...
/* Replace this with a way to have two systematics
   #1 for the ratio of B to B0
   #2 for the ratio of Bs to B0
   -- Each of these ratios could float, but are they correlated??
   so that the systematics are just the relative amounts
   and that the total comb. background contribution in Ncomb
*/

const double b0cs = 0.4354;
const double b0cs_min = 0.9;
const double b0cs_max = 1.1;

const double bcs = 0.4446;
const double bcs_min = 0.9;
const double bcs_max = 1.1;

const double bscs = 0.1200;
const double bscs_min = 0.9;
const double bscs_max = 1.1;

void SetNuisanceConstant(ModelConfig* mc, TString str) {
  ((RooRealVar*)(mc->GetNuisanceParameters()->find(str)))->setConstant(kTRUE);
}

Double_t GetNuisanceVal(ModelConfig* mc, TString str) {
...
void MakeJpsiTauModel() {
  ...
  double mcN_sigtau_2012,mcN_sigmu_2012,mcN_b0comb_2012,mcN_bcomb_2012,mcN_bscomb_2012,mcN_psi2smu_2012,ddN_misID_2012;
  ...
  double *ddnorms[numDataDrivenSamples] = {
    &ddN_misID_2012
  };
  ...
  for (int i = 0; i < numDataDrivenSamples; i++) {
    file.GetObject("h_"+datadrivenhistos[i],htemp);
    assert(htemp!=NULL);
    *(ddnorms[i])=1./htemp->Integral();
    cout << "mcN_" << datadrivenhistos[i] << " = " << 1./ *(ddnorms[i]) << endl;
  }
  file.Close();


  double expTau = expMu * tau_to_mu * expRawR;
  double expMisID = RelLumi / ddN_misID_2012;

  cout << "Expected number of Tau events: " << expTau << endl;
  cout << "Expected number of Mu events: " << expMu << endl;
  cout << "Expected number of Comb events: " << expComb << endl;
  cout << "Expected number of misID events: " << expMisID << endl;
  ...
  */
  ...
  RooStats::HistFactory::Channel chan_2012("chan2012");
  chan_2012.SetStatErrorConfig(0.1,"Poisson");
  chan_2012.SetData("h_data_2012",locData.Data());
  ...
  RooStats::HistFactory::Sample* samps[numChannels]; // placeholder
  ...
  /********************************** Bc -> J/ψ μ ν (NORM) ****************************************/
   ...
   for (int i = 0; i < numChannels; i++) {
     ...
     samps[i]->AddNormFactor("mcNorm_sigmu_2012",mcN_sigmu_2012,1e-9,1.); // reset to 1
  ...
  for (int i = 0; i < numChannels; i++) {
    samps[i]->SetNormalizeByTheory(kFALSE);
    samps[i]->AddNormFactor("mcNorm_sigtau_2012",mcN_sigtau_2012,1e-9,1.); // reset to 1
 ...
 /********************************** Bc -> ψ(2S) μ ν (FEEDDOWN) ***********************************************/
  ...
  for (int i = 0; i < numChannels; i++) {
    ...
    if (!constrainFeeddown) {
      ...
      samps[i]->AddNormFactor("f_psi2s",0.2,0.2,0.2); // branching Bc -> ψ(2S) μ ν relative to Bc -> J/ψ μ ν -- take from Ebert, Kiselev, etc. // find a better name
      ...
      samps[i]->AddOverallSys("BF_psi2sFD",psi2s_to_jpsi_min,psi2s_to_jpsi_max); // PDG error in above
    }
    chans[i]->AddSample(*samps[i]); 
  }
  ...
  /********************************** Combinatoric B0 BKG (FROM MC, SADLY) *******************************************/
   ...
   misID.AddNormFactor("NmisID",RelLumi,1e-6,1e6);
...
//   chan_2012.AddSample(SB);
  ...
  // Get the model and R(D*) a.k.a. POI from HistFactory manually and steer RooFit myself

  RooWorkspace* w = RooStats::HistFactory::MakeModelAndMeasurementFast(meas); 
	RooAbsData *data = (RooAbsData*) w->data("obsData");

  // MODEL, POI, AND OBSERVABLES
  ...
  RooRealVar *mm2 = (RooRealVar*) obs->find("obs_x_chan2012");
  mm2->SetTitle("m^{2}_{miss}");
  mm2->setUnit("GeV^{2}");
  RooRealVar *loglt = (RooRealVar*) obs->find("obs_y_chan2012");
  loglt->SetTitle("ln(tau/ps)");
  loglt->setUnit("");
  RooRealVar *q2 = (RooRealVar*) obs->find("obs_z_chan2012");
  q2->SetTitle("q^{2}");
  q2->setUnit("MeV^{2}");
  ...
  SetNuisanceConstant(mc,"f_b0cs");
  SetNuisanceConstant(mc,"f_bcs");
  SetNuisanceConstant(mc,"f_bscs");
//  SetNuisanceConstant(mc,"BF_b0cs");
//  SetNuisanceConstant(mc,"BF_bcs");
//  SetNuisanceConstant(mc,"BF_bscs");
  if (constrainFeeddown) {
  ...
  RooProdPdf *channelPDF = (RooProdPdf*) w->obj("model_chan2012");
...
//  RooDataSet* data = model->generate(*obs,num);

  RooPlot* RawR_pulls;

  if (toyMC) {
		RooMCStudy* mcs = new RooMCStudy(*model,*model,*obs,"","");
  	Int_t num = floor(expTau + expMu + expComb + expMisID);
		mcs->generateAndFit(numToys,num,true);
		data = (RooAbsData*) mcs->genData(0);
		RawR_pulls = mcs->plotPull(*poi,-5,5,40,kTRUE);
  }
  ...
  // BINNED PLOTS
	/* Temporarily replacing this with lof(lifetime) */
...
//  }
	RooPlot *mm2q2_frame[loglt_bins]; // mm2 in bins of q2
  RooPlot *logltq2_frame[loglt_bins]; // loglt in bins of q2
  RooPlot *q2frames[2*loglt_bins]; // Combination of the two above
  for (int i = 0; i < loglt_bins; i++) {
    mm2q2_frame[i] = mm2->frame();
    logltq2_frame[i] = q2->frame();
    q2frames[i] = mm2q2_frame[i];
    q2frames[i+loglt_bins] = logltq2_frame[i];
  }
  ...
  for (int i = 0; i < nframes; i++) {
    ...
    model->plotOn(drawframes[i],Slice(*idx),ProjWData(*idx,*data),LineColor(colors[0]),FillColor(colors[0]),DrawOption("F"));
    resids[i] = drawframes[i]->pullHist();
    if (!BeestonBarlow) {
      for (int j = 1; j < ncomps; j++)
	model->plotOn(drawframes[i],Slice(*idx),ProjWData(*idx,*data),Components(stackparts[j]),LineColor(colors[j]),FillColor(colors[j]),DrawOption("F"));
    }
#endif
#ifndef UNBLIND // OR PLOT BLIND
    model->plotOn(drawframes[i],Slice(*idx),ProjWData(*idx,*data));
    resids[i] = drawframes[i]->pullHist();
    if (!BeestonBarlow) {
      for (int j = 1; j < ncomps_bl; j++)
	model->plotOn(drawframes[i],Slice(*idx),ProjWData(*idx,*data),Components(stackparts_bl[j]),LineColor(colors_bl[j]),LineStyle(kDashed));
    }
#endif
...
//    sprintf(cutstrings[i],"obs_z_chan2012 > %f && obs_z_chan2012 < %f",q2_low+i*(q2_high-q2_low)/q2_bins,q2_low+(i+1)*(q2_high-q2_low)/q2_bins);
//    sprintf(rangenames[i],"q2bin_%d",i);
//    q2->setRange(rangenames[i],binlow,binhigh);
//  }
  char cutstrings[loglt_bins][128];
  char rangenames[loglt_bins][32];
  for (int i = 0; i < loglt_bins; i++) {
    double binlow = loglt_low+i*(loglt_high-loglt_low)/loglt_bins;
    double binhigh = loglt_low+(i+1)*(loglt_high-loglt_low)/loglt_bins;
    sprintf(cutstrings[i],"obs_y_chan2012 > %f && obs_y_chan2012 < %f",binlow,binhigh);
    sprintf(rangenames[i],"logltbin_%d",i);
    loglt->setRange(rangenames[i],binlow,binhigh);
  }
...
//    for (int i = 0; i < q2_bins; i++) {
    for (int i = 0; i < loglt_bins; i++) {
      q2cutdata[i] = data->reduce(cutstrings[i]);
      data->plotOn(mm2q2_frame[i],Cut(cutstrings[i]),DataError(RooAbsData::Poisson));
      data->plotOn(logltq2_frame[i],Cut(cutstrings[i]),DataError(RooAbsData::Poisson));
  ...
  // pulls
//  TCanvas *c_pulls = new TCanvas("c_pulls","c_pulls",600,400);
//  c_pulls->Divide(1,1);
//  c_pulls->cd(1);
//  RawR_pulls->Draw();
...
//    c_binned->Divide(q2_bins,2);
    c_binned->Divide(loglt_bins,2);
    
//    for (int k = 0 ; k < q2_bins*2; k++) {
    for (int k = 0 ; k < loglt_bins*2; k++) {
      c_binned->cd(k+1);
      q2frames[k]->Draw();
    }
  }
...
//    RooRealVar *finalBFD1p = (RooRealVar*)((result->floatParsFinal()).find("alpha_BF_tauFD"));
    ...
    if (poi->getVal() > 1e-4 && poi->getVal() < 0.5) {printf("Non-crazy value found!\n");}
#endif
    printf("N(Bc -> J/ψ μ ν) \t\t= %f +/- %f\n",finalNmu->getVal(),finalNmu->getError());
		printf("expected: %f",expMu);
