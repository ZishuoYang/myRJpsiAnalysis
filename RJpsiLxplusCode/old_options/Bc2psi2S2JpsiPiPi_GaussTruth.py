...
mudecay = "[B_c+ -> (^psi(2S) -> (^J/psi(1S) -> ^mu+ ^mu- " + gammas + ") ^pi+ ^pi- ) ^mu+ ^nu_mu" + gammas + "]cc"
MCMuTuple = MCDecayTreeTuple("MCBc2JpsiMuTuple")
MCMuTuple.TupleName = "MCMuDecayTree"
MCMuTuple.Decay = mudecay
MCMuTuple.addBranches({
    "Bc" : "[B_c+]cc : [B_c+ -> ?]cc"
    , "psi2S" : "[B_c+ -> (^psi(2S) -> (J/psi(1S) -> mu+ mu- " + gammas + ") pi+ pi- ) mu+ nu_mu " + gammas + "]cc"
    , "Jpsi" : "[B_c+ -> (psi(2S) -> (^J/psi(1S) -> mu+ mu- " + gammas + ") pi+ pi- ) mu+ nu_mu " + gammas + "]cc"
    , "MuP" : "[B_c+ -> (psi(2S) -> (J/psi(1S) -> ^mu+ mu- " + gammas + ") pi+ pi- ) mu+ nu_mu " + gammas + "]cc"
    , "MuM" : "[B_c+ -> (psi(2S) -> (J/psi(1S) -> mu+ ^mu- " + gammas + ") pi+ pi- ) mu+ nu_mu " + gammas + "]cc"
    , "BachMu" : "[B_c+ -> (psi(2S) -> (J/psi(1S) -> mu+ mu- " + gammas + ") pi+ pi- ) ^mu+ nu_mu " + gammas + "]cc"
    , "NuMu" : "[B_c+ -> (psi(2S) -> (J/psi(1S) -> mu+ mu- "+ gammas + ") pi+ pi- ) mu+ ^nu_mu " + gammas + "]cc"
    })
MCMuTuple.ToolList = []
