########################################################################
# Y -> Jpsi Tau, Jpsi -> Mu Mu, Tau -> Mu (B_c, missing neutrinos)
#
...
DaVinci().HistogramFile = "Bc2JpsiTau_TMer_histos.root"
DaVinci().TupleFile = "Bc2JpsiTau_TMer.root"
...
MyTMJpsiAlg.Code = "mcMatch('[B_c+ => ^J/psi(1S) tau+ nu_tau]CC')"
...
MyTMMuonAlg.Code = "mcMatch('[B_c+ => J/psi(1S) (tau+ => ^mu+ nu_mu nu_tau~) nu_tau]CC')"
...
MyTMBcAlg.MotherCut = "mcMatch('[B_c+ => J/psi(1S) tau+ nu_tau]CC')"
...
tuple.Bc.LHTT.Preambulo += [
    ...
    ," JpsiX = mcMatch['Beauty -> J/psi(1S) ...']"
    , "Psi2SX = mcMatch['Beauty -> psi(2S) ...']"
    , "HasK0 = mcMatch['Beauty -> K0 ...']"
    ]
tuple.Bc.LHTT.Preambulo = [ "from LoKiPhysMC.decorators import *" , "from PartProp.Nodes import CC" ] 
tuple.Bc.LHTT.Variables = {
    ...
    , "IsABu" : "switch(mcMatch('[B0]CC'),1,0)"
