...
void ExtraDalitzAndLifetimeFitter_3D ()
{
  ...
  lt = new RooRealVar (ltstr,"lt",0,1.5);
  ...
  RooArgSet fvarsset(*m1,*m2,*lt);
  fit_data = new RooDataSet("jpsi_fit_data","jpsi_fit_data",fvarsset,Import(*w_data));
  ...
  sig_data = new RooDataSet("sig_data","sig_data",fvarsset,Import(*w_data),Cut(sig_cut.c_str()));
  b0bkg_data = new RooDataSet("b0bkg_data","b0bkg_data",fvarsset,Import(*w_data),Cut("Bc_BKG_B02JpsiX>0"));
  bbkg_data = new RooDataSet("bbkg_data","bbkg_data",fvarsset,Import(*w_data),Cut("Bc_BKG_B2JpsiX>0"));
  comb_data = new RooDataSet("comb_data","comb_data",fvarsset,Import(*w_data),Cut("(BBBKG>0)"));
  lowmass_data = new RooDataSet("lowmass_data","lowmass_data",fvarsset,Import(*w_data),Cut("(LowMassBKG>0)"));
  other_data = new RooDataSet("other_data","other_data",fvarsset,Import(*w_data),Cut(other_cut.c_str()));
  
  RooArgList fvarslist(*m1,*m2,*lt);
  sig_pdf = new RooNDKeysPdf ("sig","bm sig pdf",fvarslist,*sig_data);
  b0bkg_pdf = new RooNDKeysPdf ("b0bkg","bm b0bkg pdf",fvarslist,*b0bkg_data);
  bbkg_pdf = new RooNDKeysPdf ("bbkg","bm bbkg pdf",fvarslist,*bbkg_data);
  comb_pdf = new RooNDKeysPdf ("comb","bm comb pdf",fvarslist,*comb_data);
  lowmass_pdf = new RooNDKeysPdf ("lowmass","bm lowmass pdf",fvarslist,*lowmass_data);
  other_pdf = new RooNDKeysPdf ("other","bm other pdf",fvarslist,*other_data);
  ...
  fit_data->plotOn(ltframe);
  model->plotOn(ltframe);
  model->plotOn(ltframe,Components("sig"),LineStyle(kDashed),LineColor(kRed));
  model->plotOn(ltframe,Components("b0bkg"),LineStyle(kDashed),LineColor(kMagenta));
  model->plotOn(ltframe,Components("bbkg"),LineStyle(kDashed),LineColor(kOrange));
  model->plotOn(ltframe,Components("comb"),LineStyle(kDashed),LineColor(kCyan));
  model->plotOn(ltframe,Components("lowmass"),LineStyle(kDashed),LineColor(kGreen));
  model->plotOn(ltframe,Components("other"),LineStyle(kDashed),LineColor(kPink));

  canvas = new TCanvas("fit_canvas","fit_canvas",800,800);
  canvas->Divide(2);
  canvas->cd(1);
  ...
  canvas->cd(3);
  gPad->SetLeftMargin(0.15);
  ltframe->GetYaxis()->SetTitleOffset(1.4);
  ltframe->Draw();
