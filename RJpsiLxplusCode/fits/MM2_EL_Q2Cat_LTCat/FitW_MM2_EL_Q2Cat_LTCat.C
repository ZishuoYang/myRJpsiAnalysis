...
void FitW_MM2_EL_Q2Cat_LTCat ()
{
  ...
  lt = new RooRealVar (ltstr,"lt",0.1,8.0);
  ...
  RooThresholdCategory* ltthrcat = new RooThresholdCategory("ltthrcat","ltthrcat",*lt,"Long");
  ltthrcat->addThreshold(1.5,"Long");
  ltthrcat->addThreshold(0.5,"Medium");
  ltthrcat->addThreshold(0.1,"Short");
  w = new RooRealVar (wstr,"w",0.0,10.0);
  MyBkgTag = new RooRealVar(bkgstr,"tag",0.0);

  RooArgSet vars(*mm2,*el,*q2,*w,*MyBkgTag);
  ...
  ltcat = (RooCategory*)w_data->addColumn(*ltthrcat);
  ...
  RooArgSet fvarsset(*mm2,*el,*q2cat,*ltcat);
