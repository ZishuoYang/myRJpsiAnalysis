...
void Plot_MM2_EL_Q2Cat_LTCat ()
{
  ...
  elframe = el->frame(el->getBins());
  elframe->SetTitle("CDFLifetime (last fit)");
  elframe->SetXTitle("exp(-el/0.453ps)");
  gd->plotOn(elframe,Name("eldata"));
  model->plotOn(elframe,Name("elfit"));
  model->plotOn(elframe,LineColor(kGreen),Components("mu"));
  model->plotOn(elframe,LineColor(kRed),Components("tau"));
  model->plotOn(elframe,LineColor(kMagenta),Components("b0"));
  model->plotOn(elframe,LineColor(kCyan),Components("b"));
  model->plotOn(elframe,LineColor(kSpring),Components("comb"));
  elpulls_frame = el->frame(el->getBins());
  elpulls_frame->SetTitle("");
  elpulls_frame->SetXTitle("exp(-el/0.453ps)");
  elpulls_frame->SetYTitle("pulls");
  RooHist* elpulls = elframe->pullHist("eldata","elfit");
  elpulls_frame->addObject((TObject*)elpulls,"XP");
  elpulls_frame->SetMinimum(-5);
  elpulls_frame->SetMaximum(5);

  el_canvas = new TCanvas("el_canvas","el_canvas",800,800);
  el_canvas->cd();
  TPad* elhistpad = new TPad("elhistpad","elhistpad",0.0,0.25,1.0,1.0);
  elhistpad->Draw();
  elhistpad->SetTopMargin(0.1);
  TPad* elpullpad = new TPad("elpullpad","elpullpad",0.0,0.0,1.0,0.25);
  elpullpad->Draw();
  elpullpad->SetTopMargin(0.1);
  elhistpad->cd();
  elframe->SetTitleFont(63,"XY");
  elframe->SetTitleSize(18,"XY");
  elframe->SetTitleOffset(2,"XY");
  elframe->SetLabelFont(63,"XY");
  elframe->SetLabelSize(14,"XY");
  elframe->Draw();
  elhistpad->Update();
  elpullpad->cd();
  elpulls_frame->SetTitleFont(63,"XY");
  elpulls_frame->SetTitleSize(18,"XY");
  elpulls_frame->SetTitleOffset(2,"XY");
  elpulls_frame->SetLabelFont(63,"XY");
  elpulls_frame->SetLabelSize(14,"XY");
  elpulls_frame->Draw();
  elpullpad->Update();
  el_canvas->Update();
