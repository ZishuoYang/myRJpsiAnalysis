...
def getJpsiKUnbiasRecTuple(simulation,outputLocation):
    ...
    JpsiKUnbiasRecTuple.addBranches({
        "B0" : "[^(B0 -> (B+ -> (J/psi(1S) -> mu+ mu-) K+) pi-)]CC"
        , "Bp" : "[B0 -> ^(B+ -> (J/psi(1S) -> mu+ mu-) K+) pi-]CC"
        , "Jpsi" : "[B0 -> (B+ -> ^(J/psi(1S) -> mu+ mu-) K+) pi-]CC"
        , "MuP" : "[B0 -> (B+ -> (J/psi(1S) -> ^mu+ mu-) K+) pi-]CC"
        , "MuM" : "[B0 -> (B+ -> (J/psi(1S) -> mu+ ^mu-) K+) pi-]CC"
        , "Kaon" : "[B0 -> (B+ -> (J/psi(1S) -> mu+ mu-) ^K+) pi-]CC"
        , "Pion" : "[B0 -> (B+ -> (J/psi(1S) -> mu+ mu-) ^K+) pi-]CC"
        })
