...
void Psi2sFit() {
  ...
  // DATA
  Double_t lowRange = 3500; // to throw out shoulder
  Double_t highRange = 3900;
  RooRealVar mass("Psi_MM", "Psi_MM", lowRange, highRange);
  RooRealVar jpsiMass("Jpsi_MM", "jpsi_MM",0);//2950,3250);
  RooRealVar dira("Psi_DIRA","Psi_DIRA",0);//-1.0,1.0);
  RooRealVar vchi2("Psi_VCHI2","Psi_VCHI2",0);//0.0,50.0);
  RooRealVar ipchi2("Psi_IPCHI2","Psi_IPCHI2",0);//0.0,50.0);
  ...
  // SIGNAL MODEL
  RooRealVar mean("mean","mean",3690,3500,3800);
  RooRealVar sigma("sigma","sigma",27.5,1.0,50.0);
  RooRealVar mean2("mean2","mean2",3690,3500,3800);
  RooRealVar mult("mult","mult",0.45,0.1,1.0);
  ...
  RooRealVar sigfrac("sigfrac","sigfrac",0.47,0.05,1.0);
  RooAddPdf mass_sig("mass_sig","mass_sig",RooArgList(g1,g2),RooArgList(sigfrac));

  // BACKGROUND MODEL
  RooRealVar p1("p1","p1",1.6,-5,5);
  RooRealVar p2("p2","p2",0,-5,5);
  RooRealVar p3("p3","p3",0,-5,5);
  RooRealVar p4("p4","p4",0,-5,5);
  RooConstVar mid("mid","mid",(lowRange+highRange)/2.0);
  RooConstVar width("width","width",highRange-lowRange);
  RooFormulaVar adj("adj","adj","(Psi_MM-mid)/width",RooArgList(mass,mid,width));
  RooGenericPdf mass_bkg("mass_bkg","mass_bkg",
                         "1.0+p1*adj+p2*adj*adj+p3*adj*adj*adj + p4*adj*adj*adj*adj",
                         RooArgList(adj,p1,p2,p3,p4));

  // COMBINED MODEL
  RooRealVar nsig("nsig","nsig", 6.0e4, 1, 1e6);
  RooRealVar nbkg("nbkg","nbkg", 3.2e5, 1, 1e7);
  RooAddPdf model("model","model",
                  RooArgList(mass_sig, mass_bkg), 
                  RooArgList(nsig,nbkg));
  
  RooArgSet vars1(mass,jpsiMass,dira,vchi2,ipchi2,nTracks);
  RooArgSet vars2(Pi_P,Pi_PT,Pi_ETA);
  ...
  RooArgSet vars46(vars4,vars6);
  RooArgSet vars(vars12,vars46);

  // ADD DATA
  std::cout << "Add data" << std::endl;

  TFile* file = new TFile("Bc_Psi2S_Cal_SELECTION.root");
  TTree* tree = (TTree*)file->Get("DecayTree");
  RooDataSet *data = new RooDataSet("data","data",tree,vars,"Psi_VCHI2<9 && Psi_IPCHI2>9 && abs(Jpsi_MM-3096.916)<50.0");
  ...
  // RooDataSet* reduced = (RooDataSet*)Sdata->reduce("abs(Psi_MM-3690)<100.0");
