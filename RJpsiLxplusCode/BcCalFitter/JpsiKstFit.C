...
void JpsiKstFit() {
  ...
  RooRealVar kstMass("Kst_MM", "kst_MM",0);//2950,3250);
  ...
  RooRealVar K_P("K_P","K_P",0);//0.0,2.0e5);
  RooRealVar K_PT("K_PT","K_PT",0);//0.0,2.5e4);
  RooRealVar K_ETA("K_Eta","K_Eta",0);//0.0,6.0);

  RooRealVar K_DLLK("K_CombDLLK","K_CombDLLK",0);//-1100,200);
  RooRealVar K_DLLp("K_CombDLLp","K_CombDLLp",0);//-1100,200);
  RooRealVar K_DLLe("K_CombDLLe","K_CombDLLe",0);//-1100,200);
  RooRealVar K_DLLmu("K_CombDLLmu","K_combDLLmu",0);//-1100,200);

  RooRealVar K_isMuon("K_IsMuon","K_IsMuon",0);//0.0,1.0);
  RooRealVar K_isMuonLoose("K_IsMuonLoose","K_IsMuonLoose",0);//0.0,1.0);
  RooRealVar K_nShared("K_nShared","K_nShared",0);//0.0,100.0);

  RooRealVar K_probK("K_ProbNNk","K_ProbNNk",0);//0.0,1.0);
  RooRealVar K_probPi("K_ProbNNpi","K_ProbNNpi",0);//0.0,1.0);
  RooRealVar K_probP("K_ProbNNp","K_ProbNNp",0);//0.0,1.0);
  RooRealVar K_probMu("K_ProbNNmu","K_ProbNNmu",0);//0.0,1.0);
  RooRealVar K_probE("K_ProbNNe","K_ProbNNe",0);//0.0,1.0);

  RooRealVar K_inMuon("K_InMuonAcc","K_InMuonAcc",0);//0.0,1.0);
  // RooRealVar K_region("K_regionInM2","K_regionInM2",0);//-1.0,4.0);
  // RooRealVar K_muondist2("K_MuonDist2","K_MuonDist2",0);//-1.1e5,1e3);
  // RooRealVar K_mchi2("K_MatchCHI2","K_MatchCHI2",0);//0.0,100.0);

  RooRealVar Pi_P("Pi_P","Pi_P",0);//0.0,2.0e5);
  RooRealVar Pi_PT("Pi_PT","Pi_PT",0);//0.0,2.5e4);
  RooRealVar Pi_ETA("Pi_Eta","Pi_Eta",0);//0.0,6.0);

  RooRealVar Pi_DLLK("Pi_CombDLLK","Pi_CombDLLK",0);//-1100,200);
  RooRealVar Pi_DLLp("Pi_CombDLLp","Pi_CombDLLp",0);//-1100,200);
  RooRealVar Pi_DLLe("Pi_CombDLLe","Pi_CombDLLe",0);//-1100,200);
  RooRealVar Pi_DLLmu("Pi_CombDLLmu","Pi_combDLLmu",0);//-1100,200);

  RooRealVar Pi_isMuon("Pi_IsMuon","Pi_IsMuon",0);//0.0,1.0);
  RooRealVar Pi_isMuonLoose("Pi_IsMuonLoose","Pi_IsMuonLoose",0);//0.0,1.0);
  RooRealVar Pi_nShared("Pi_nShared","Pi_nShared",0);//0.0,100.0);

  RooRealVar Pi_probK("Pi_ProbNNk","Pi_ProbNNk",0);//0.0,1.0);
  RooRealVar Pi_probPi("Pi_ProbNNpi","Pi_ProbNNpi",0);//0.0,1.0);
  RooRealVar Pi_probP("Pi_ProbNNp","Pi_ProbNNp",0);//0.0,1.0);
  RooRealVar Pi_probMu("Pi_ProbNNmu","Pi_ProbNNmu",0);//0.0,1.0);
  RooRealVar Pi_probE("Pi_ProbNNe","Pi_ProbNNe",0);//0.0,1.0);

  RooRealVar Pi_inMuon("Pi_InMuonAcc","Pi_InMuonAcc",0);//0.0,1.0);
  // RooRealVar Pi_region("Pi_regionInM2","Pi_regionInM2",0);//-1.0,4.0);
  // RooRealVar Pi_muondist2("Pi_MuonDist2","Pi_MuonDist2",0);//-1.1e5,1e3);
  // RooRealVar Pi_mchi2("Pi_MatchCHI2","Pi_MatchCHI2",0);//0.0,100.0);
  ...
  RooArgSet vars1(mass,jpsiMass,kstMass,dira,vchi2,nTracks);
  RooArgSet vars2(K_P,K_PT,K_ETA,Pi_P,Pi_PT,Pi_ETA);
  RooArgSet vars3(K_DLLK,K_DLLp,K_DLLe,K_DLLmu,K_isMuon,K_isMuonLoose,K_nShared);
  RooArgSet vars4(Pi_DLLK,Pi_DLLp,Pi_DLLe,Pi_DLLmu,Pi_isMuon,Pi_isMuonLoose,Pi_nShared);
  RooArgSet vars5(K_probK,K_probPi,K_probP,K_probMu,K_probE,K_inMuon);//,region,muondist2,mchi2);
  RooArgSet vars6(Pi_probK,Pi_probPi,Pi_probP,Pi_probMu,Pi_probE,Pi_inMuon);//,region,muondist2,mchi2);
  RooArgSet vars12(vars1,vars2);
  RooArgSet vars34(vars3,vars4);
  RooArgSet vars56(vars5,vars6);
  RooArgSet vars1234(vars12,vars34);
  RooArgSet vars(vars1234,vars56);

  // ADD DATA
  std::cout << "Add data" << std::endl;

  TFile* file = new TFile("Bc_JpsiKst_Cal_SELECTION.root");
  TTree* tree = (TTree*)file->Get("DecayTree");
  RooDataSet *data = new RooDataSet("data","data",tree,vars,"log(1.0-B_DIRA)<-10.0 && B_VCHI2<25 && abs(Jpsi_MM-3096.916)<50.0 && abs(Kst_MM-891.66)<50.0");
