...
void HistFactJpsi_new() {
 ...
 double *mcnorms[8]={&mcN_sigmu,&mcN_sigtau,&mcN_Psi2SMu,&mcN_JpsiDx,&mcN_ChicMu,&mcN_misID,&mcN_fakeJpsi,&mcN_Bd2JpsiX};
  ...
  chan.SetData("h_data_subset", "Histos_20211030.root");
  ...
  RooStats::HistFactory::Sample sigmu("h_sigmu","h_sigmu", "Histos_20211030.root");
  ...
  RooStats::HistFactory::Sample sigtau("h_sigtau","h_sigtau", "Histos_20211030.root");
  ...
  RooStats::HistFactory::Sample Psi2SMu("h_Psi2SMu","h_Psi2SMu", "Histos_20211030.root");
 ...
 /************************* Bc->JpsiDx *******************************/
  ...
  RooStats::HistFactory::Sample JpsiDx("h_JpsiDx","h_JpsiDx", "Histos_20211030.root");
  if(BBon3d) JpsiDx.ActivateStatError();
  JpsiDx.SetNormalizeByTheory(kFALSE);
  JpsiDx.AddNormFactor("mcNorm_JpsiDx", mcN_JpsiDx, 1e-9, 1.);
  JpsiDx.AddNormFactor("NJpsiDx",1e2*0.8,1e-6,1e2);
  chan.AddSample(JpsiDx);
 
 /************************* Bc->Chi_c{1,2}munu *******************************/

  RooStats::HistFactory::Sample ChicMu("h_ChiMu","h_ChiMu", "Histos_20211030.root");
 ...
 /************************* Bu,d->JpsiX *******************************/
  ...
  RooStats::HistFactory::Sample Bd2JpsiX("h_Bd2JpsiX","h_Bd2JpsiX", "Histos_20211030.root");
  ...
  RooStats::HistFactory::Sample misID("h_misID","h_misID_smear", "Histos_20211030.root");
  ...
  if (floatMisIDSmear){ // MisID smearing
	misID.AddHistoSys("misID_smear","h_misID","Histos_20211030.root","","h_misID_doublesmear","Histos_20211030.root","");
  }
  misID.AddNormFactor("mcNorm_misID", mcN_misID, 1e-9, 1.);
  misID.AddNormFactor("NmisID",2e3*0.8,1e-6,3e3);
  chan.AddSample(misID);

  /*********************** Fake Jpsi BKG (FROM DATA)  *******************************/

  RooStats::HistFactory::Sample fakeJpsi("h_fakeJpsi","h_fakeJpsi", "Histos_20211030.root");
...
if(result != NULL)
    ...
    if (toyMC) 
    {
      ...
      for (int i = 0; i < nframes; i++){
        ...
        model_hf->plotOn(drawframes[i], Slice(*idx),ProjWData(*idx,*data),DrawOption("F"),FillColor(kOrange+2),Components("*misID*,*sigmu*,*Psi2SMu*,*JpsiDx*,*fakeJpsi*,*ChicMu*,*Bd2JpsiX*"));
        model_hf->plotOn(drawframes[i], Slice(*idx),ProjWData(*idx,*data),DrawOption("F"),FillColor(kGreen+1),Components("*misID*,*sigmu*,*Psi2SMu*,*JpsiDx*,*fakeJpsi*,*ChicMu*"));
      ...
      if(slowplots == true)
      {
        ...
        for (int i = 0; i < q2_bins; i++)
        {
          ...
          model_hf->plotOn(mm2q2_frame[i], Slice(*idx),ProjWData(*idx,*data),ProjectionRange(rangenames[i]),DrawOption("F"),FillColor(kOrange+2),Components("*sigmu*,*Psi2SMu*,*JpsiDx*,*misID*,*fakeJpsi*,*ChicMu*,*Bd2JpsiX*"));
          model_hf->plotOn(Elq2_frame[i], Slice(*idx),ProjWData(*idx,*data),ProjectionRange(rangenames[i]), DrawOption("F"),FillColor(kOrange+2),Components("*sigmu*,*Psi2SMu*,*JpsiDx*,*misID*,*fakeJpsi*,*ChicMu*,*Bd2JpsiX*"));
          model_hf->plotOn(mm2q2_frame[i], Slice(*idx),ProjWData(*idx,*data),ProjectionRange(rangenames[i]),DrawOption("F"),FillColor(kGreen+1),Components("*sigmu*,*Psi2SMu*,*JpsiDx*,*misID*,*fakeJpsi*,*ChicMu*"));
          model_hf->plotOn(Elq2_frame[i], Slice(*idx),ProjWData(*idx,*data),ProjectionRange(rangenames[i]), DrawOption("F"),FillColor(kGreen+1),Components("*sigmu*,*Psi2SMu*,*JpsiDx*,*misID*,*fakeJpsi*,*ChicMu*"));
